...
internal class DefaultDocumentWriter(CodeTarget codeTarget, RazorCodeGenerationOptions options) : DocumentWriter
{
    ...
    private sealed class Visitor(CodeTarget codeTarget, CodeRenderingContext context) : IntermediateNodeVisitor
    {
        ...
        public override void VisitMethodDeclaration(MethodDeclarationIntermediateNode node)
        {
            ...
            writer.Write($"{node.ReturnTypeName} ");
            ...
            foreach (var parameter in node.Parameters)
            {
                ...
                if (!parameter.Modifiers.IsDefaultOrEmpty)
                {
                ...
                writer.Write($"{parameter.TypeName} {parameter.Name}");
            }
        ...
        {
            CodeWriter.WriteField(node.SuppressWarnings, node.Modifiers, node.TypeName, node.Name);
        }

        public override void VisitPropertyDeclaration(PropertyDeclarationIntermediateNode node)
        {
            CodeWriter.WritePropertyDeclaration(node.Modifiers, node.ReturnTypeName, node.Name, node.ExpressionBody, _context);
        }
