...
internal abstract class LazyContent
{
    ...
    public abstract Content Value { get; }

    public static LazyContent Create<T>(T arg, Func<T, Content> contentFactory)
    {
    ...
    private sealed class LazyContentImpl<T>(T arg, Func<T, Content> contentFactory) : LazyContent
    {
        ...
        private T _arg = arg;
        private Func<T, Content> _contentFactory = contentFactory;

        private Content _value;
        private int _state;

        public override Content Value
        {
            get
            {
        ...
        private Content GetOrComputeAndStoreValue()
        {
            ...
            while (true)
            {
                switch (Interlocked.CompareExchange(ref _state, Computing, Uninitialized))
                {
                    ...
                    default:
                        return Assumed.Unreachable<Content>();
                }
            }
        }
    }
