...
internal abstract partial class SyntaxNode(GreenNode green, SyntaxNode parent, int position)
{
    ...
    {
        using PooledArrayBuilder<OldSyntaxToken> tokens = [];
    ...
    public IEnumerable<OldSyntaxToken> DescendantTokens(Func<SyntaxNode, bool>? descendIntoChildren = null)
    {
        return DescendantNodesImpl(Span, descendIntoChildren, includeSelf: true).OfType<OldSyntaxToken>();
    }
    ...
    protected internal abstract SyntaxNode ReplaceTokenInListCore(OldSyntaxToken originalToken, IEnumerable<OldSyntaxToken> newTokens);
    protected internal abstract SyntaxNode InsertTokensInListCore(OldSyntaxToken originalToken, IEnumerable<OldSyntaxToken> newTokens, bool insertBefore);
