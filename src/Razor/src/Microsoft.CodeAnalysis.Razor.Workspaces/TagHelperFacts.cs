...
internal static class TagHelperFacts
{
        ...
        string? parentTag)
    {
        ...
        using var matchingTagHelpers = new PooledArrayBuilder<TagHelperDescriptor>();
        ...
        foreach (var tagHelper in tagHelpers)
        {
            foreach (var rule in tagHelper.TagMatchingRules)
            {
                ...
                {
                    matchingTagHelpers.Add(tagHelper);
                    break;
                }
            }
        }

        return matchingTagHelpers.ToImmutableAndClear();
    }

    public static ImmutableArray<TagHelperDescriptor> GetTagHelpersGivenParent(
        TagHelperDocumentContext documentContext,
        string? parentTag)
    {
        ArgHelper.ThrowIfNull(documentContext);
