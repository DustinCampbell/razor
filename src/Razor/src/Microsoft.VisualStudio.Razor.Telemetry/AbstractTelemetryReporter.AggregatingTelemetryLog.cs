...
internal abstract partial class AbstractTelemetryReporter
{
    ...
    /// metric name logged.
    /// </summary>
    private sealed class AggregatingTelemetryLog
    {
        ...
        private readonly AbstractTelemetryReporter _reporter;
        ...
        public AggregatingTelemetryLog(AbstractTelemetryReporter reporter, string name, double[]? bucketBoundaries)
        {
            var meterProvider = new VSTelemetryMeterProvider();

            _reporter = reporter;
            _meter = meterProvider.CreateMeter(TelemetryUtils.GetPropertyName("meter"), version: MeterVersion);
            _name = name;
            _flushLock = new();
        ...
        public void Log(string histogramKey, int value, string method)
        {
            ...
            {
                var telemetryEvent = TelemetryUtils.CreateTelemetryEvent(_name, new Property("method", method));
        ...
        private bool IsEnabled => _reporter.IsEnabled;
