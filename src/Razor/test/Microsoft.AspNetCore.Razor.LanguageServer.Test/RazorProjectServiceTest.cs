...
#nullable enable
    ...
    {
        // Arrange
        var hostProject = new HostProject(new ProjectKey("C:/path/to/obj"), "C:/path/to/project.csproj", RazorConfiguration.Default, "TestRootNamespace");
    ...
    public async Task IProjectInfoListener_UpdatedAsync_UpdatingDocument_MapsRelativeFilePathToActualDocument()
    {
        ...
        var newDocument = new HostDocument("file.cshtml", "file.cshtml", RazorFileKind.Component);
    ...
    public async Task IProjectInfoListener_UpdatedAsync_AddsNewDocuments()
    {
        // Arrange
        var hostProject = new HostProject(new ProjectKey("C:/path/to/obj"), "C:/path/to/project.csproj", RazorConfiguration.Default, "TestRootNamespace");
        var oldDocument = new HostDocument("C:/path/to/file.cshtml", "file.cshtml", RazorFileKind.Legacy);

        await _projectManager.UpdateAsync(updater =>
        {
            updater.AddProject(hostProject);
            updater.AddDocument(hostProject.Key, oldDocument, StrictMock.Of<TextLoader>());
        });

        var newDocument = new HostDocument("C:/path/to/file2.cshtml", "file2.cshtml", RazorFileKind.Legacy);
    ...
    public async Task IProjectInfoListener_UpdatedAsync_MovesDocumentsFromMisc()
    {
        ...
        var project = _projectManager.GetRequiredProject(hostProject.Key);
        var projectFilePaths = project.DocumentFilePaths.OrderBy(path => path);
        Assert.Equal(projectFilePaths, [hostDocument.FilePath]);
    ...
    public async Task IProjectInfoListener_UpdatedAsync_MovesDocumentsFromMisc_ViaService()
    {
        ...
        var ownerProjectKey = new ProjectKey(IntermediateOutputPath);
        await _projectService.GetTestAccessor().AddProjectAsync(
            ownerProjectKey, ProjectFilePath, RazorConfiguration.Default, RootNamespace, displayName: null, DisposalToken);
        ...
        var addedDocument = new HostDocument(DocumentFilePath, DocumentFilePath, RazorFileKind.Legacy);
    ...
    {
        // Arrange
        var hostProject = new HostProject(new ProjectKey("path/to/obj"), "path/to/project.csproj", RazorConfiguration.Default, "TestRootNamespace");
        var newDocument = new HostDocument("path/to/file.cshtml", "file.cshtml", RazorFileKind.Legacy);

        await _projectManager.UpdateAsync(updater =>
        {
            updater.AddProject(hostProject);
            updater.AddDocument(hostProject.Key, newDocument, StrictMock.Of<TextLoader>());
        });
    ...
    public async Task IProjectInfoListener_UpdatedAsync_UpdatesLegacyDocumentsAsComponents()
    {
        ...
        var newDocument = legacyDocument with { FileKind = RazorFileKind.Component };
    ...
    public async Task IProjectInfoListener_UpdatedAsync_SameConfigurationDifferentRootNamespace_UpdatesRootNamespace()
    {
        ...
        var ownerProjectKey = new ProjectKey(IntermediateOutputPath);
        await _projectService.GetTestAccessor().AddProjectAsync(
            ownerProjectKey, ProjectFilePath, RazorConfiguration.Default, rootNamespace: null, displayName: null, DisposalToken);
    ...
    public async Task CloseDocument_ClosesDocumentInAllOwnerProjects()
    {
        ...
        var ownerProjectKey1 = new ProjectKey(IntermediateOutputPath1);
        await _projectService.GetTestAccessor().AddProjectAsync(
            ownerProjectKey1, ProjectFilePath, RazorConfiguration.Default, RootNamespace, displayName: null, DisposalToken);

        var ownerProjectKey2 = new ProjectKey(IntermediateOutputPath2);
        await _projectService.GetTestAccessor().AddProjectAsync(
            ownerProjectKey2, ProjectFilePath, RazorConfiguration.Default, RootNamespace, displayName: null, DisposalToken);
        await _projectService.AddDocumentToPotentialProjectsAsync(DocumentFilePath, DisposalToken);
        await _projectService.OpenDocumentAsync(DocumentFilePath, s_emptyText, DisposalToken);
    ...
    public async Task AddDocumentToMiscProjectAsync_IgnoresKnownDocument()
    {
        ...
        await _projectService.GetTestAccessor().AddProjectAsync(
            new ProjectKey(IntermediateOutputPath), ProjectFilePath, RazorConfiguration.Default, RootNamespace, displayName: null, DisposalToken);
        await _projectService.AddDocumentToPotentialProjectsAsync(DocumentFilePath, DisposalToken);
