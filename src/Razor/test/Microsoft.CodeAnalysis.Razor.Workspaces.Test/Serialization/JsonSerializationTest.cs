...
public class JsonSerializationTest(ITestOutputHelper testOutput) : ToolingTestBase(testOutput)
{
    ...
    {
        // Arrange
        var legacyDocument = new HostDocument("/path/to/file.cshtml", "file.cshtml", RazorFileKind.Legacy);
        var componentDocument = new HostDocument("/path/to/otherfile.razor", "otherfile.razor", RazorFileKind.Component);
        var projectInfo = new RazorProjectInfo(
            new ProjectKey("/path/to/obj/"),
            "/path/to/project.csproj",
            _configuration,
