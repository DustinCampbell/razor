...
/// </remarks>
internal partial class ListPool<T> : DefaultObjectPool<List<T>>
{
    public static readonly ListPool<T> Default = new(Policy.Instance, size: DefaultPool.DefaultPoolSize);

    protected ListPool(IPooledObjectPolicy<List<T>> policy, int size)
        : base(policy, size)
    {
    }
